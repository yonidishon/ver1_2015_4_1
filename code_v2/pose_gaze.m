...
cropped_box=[nmsbbox(:,1)+bb_width./4,nmsbbox(:,2)-(1/9).*bb_height,nmsbbox(:,3)-bb_width./4,nmsbbox(:,4)-(5/9).*bb_height];
...
for ii=1:size(cropped_box,1)
    ...
    map=map+nmsconf(ii).*exp(-((X - bb_g(ii,1)).^2/2/(bb_g(ii,3)/5)^2 + (Y - bb_g(ii,2)).^2/2/(bb_g(ii,4)/5)^2));
end
map=map./(max(map(:))+eps);
end
