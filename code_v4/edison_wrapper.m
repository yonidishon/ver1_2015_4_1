function [varargout] = edison_wrapper(rgbim, featureIm,varargin)
%
% Performing mean_shift operation on image
%
% Usage:
%   [fimage labels modes regSize grad conf] = edison_wrapper(rgbim, featureImc, ...)
% 
% Inputs:
%   rgbim - original image in RGB space
%   featureImc - converting RGB to some feature space in which to perform
...
if nargout >= 2, varargout{2} = double(labels'+1); end;
if nargout >= 3, varargout{3} = modes'; end;
if nargout >= 4, varargout{4} = double(regSize)'; end;
