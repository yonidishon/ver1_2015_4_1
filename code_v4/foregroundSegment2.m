...
%                    segMap(lMap==objEdgeSegments(segI))=objEdgeSegments(segI);
%                 end
%                 figure;imagesc(segMap);axis image;colormap(rand(numel(objEdgeSegments),3));
% %         %
%                 segMap = zeros(size(lMap));
%                 for segI = 1:numel(surroundSegments)
%                    segMap(lMap==surroundSegments(segI))=surroundSegments(segI);
%                 end
...
% figure;imagesc(oMap);axis image;colormap(hot);
