...
finalResultRoot = '\\CGM10\D\Video_Saliency_Results\FinalResults4\PCA_M_MRF_v0\';
...
%         end
        ...
        try
            ...
            ONE_DEGREE_MBLKS = ONE_DEGREE_PXLS*size(predMapPCAFbest,1)/H;
            tic
            S = SalOBDL_MRF(predMapPCAFbest(:,:,1:FRMS_CNT), ONE_DEGREE_MBLKS,T_t,T_o,T_c);
            times = toc/FRMS_CNT;
            disp(times*1000)
            for ifr = 1:length(indFr)
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                [sim(:,:,ifr), outMaps] = similarityFrame3(S(:,:,indFr(ifr)), gazeData, measures, ...
                    'self',...
