...
for ii=1:length(testIdx) % run for the length of the defined exp.
    ...
    try % MAIN ROUTINE to do.
        ...
        if (~exist(fullfile(finalResultRoot_orig,videos{iv}), 'dir'))
            mkdir(fullfile(finalResultRoot_orig,videos{iv}));
        end
        m = vr.Height;
        n = vr.Width;
        ...
        if (~exist(fullfile(finalResultRoot_new,videos{iv}), 'dir'))
            mkdir(fullfile(finalResultRoot_new,videos{iv}));
        end  
        ...
        try
            ...
            for ifr = 1:length(indFr)
                ...
                feature_data=load(fullfile(finalResultRoot_orig,videos{iv},sprintf('frame_%06d.mat',frames(indFr(ifr)))));
                [X,Y]=meshgrid(1:n,1:m);
                gz.points = gazeData.points{gazeData.index};
