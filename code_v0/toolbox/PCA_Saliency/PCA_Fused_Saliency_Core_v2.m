function [result, resultD] = PCA_Fused_Saliency_Core_v2(fx,fy,I_RGB)
% Yonatan Modified 01/03/2015 - adaptation from Ran Video saliency code.
...
sError=structDifference(a-mean2(a),rad-mean2(rad),L_SPM,STATS2,SEGMENTS,numOfSegments,imSize);
sError = discardEdges(stableNormalize(sError));

rErrorMap=min(sError(:)) .* ones(imSize);
rErrorMap(5:(end-4),5:(end-4))=sError(5:(end-4),5:(end-4));
...
function [sError_fx] = structDifference(a,rad,L_SPM,STATS2,SEGMENTS,numOfSegments,imSize)
...
IX = (IX(1:round(numOfSegments/64))); % narrow to highest 25% indices
...
sError_fx =  (projNrec([pVecL_fx,pVecL_fy],IX,imSize));
end
