     ...
     
{
    ...
    for(w=0;w<numberWindows;w++)
    {
       ...
       xminSurr=floor(MAX(xmin[w]-offsetWidth,1)+0.5);
       xmaxSurr=floor(MIN(xmax[w]+offsetWidth,width)+0.5);
       yminSurr=floor(MAX(ymin[w]-offsetHeight,1)+0.5);
       ymaxSurr=floor(MIN(ymax[w]+offsetHeight,height)+0.5);
