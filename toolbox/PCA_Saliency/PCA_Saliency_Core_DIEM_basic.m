function [result resultD] = PCA_Saliency_Core_DIEM_basic(I_RGB)
% Yonatan Modified 08/01/2015 - enforce Gaussian if image is black
% Yonatan Modified 19/01/2015 - return only the Gaussians
% Yonatan Modified 13/05/2015 - Based on PCA_Saliency_Core.m
% -> Trigger for modification is baseline vs HOWMANYBITS
% List of changes:
% 1. return Guassians with result. and encorporate color
...
for th=1:-0.1:0.1
    ...
    if isempty(STATS) % no objects in the image for current th
...
% result = stableNormalize(result);
if max(result(:))==0 %black image input
    result=W;
end
% Only Result withour the weighting
