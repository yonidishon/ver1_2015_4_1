function [result resultD] = PCA_Saliency_Core_v2(I_RGB)
...
reconError1=zeros(size(reconError));
for ii=1:length(regionmeans);
    subs=(SEGMENTS==ii);
    reconError1(subs)=regionmeans(ii);
end
% rErrorMap=min(reconError(:)) .* ones(imSize);
% rErrorMap(5:(end-4),5:(end-4))=reconError(5:(end-4),5:(end-4));

rErrorMap=min(reconError1(:)) .* ones(imSize);
rErrorMap(5:(end-4),5:(end-4))=reconError1(5:(end-4),5:(end-4));

% figure();subplot(2,1,1);imshow(rErrorMap1);subplot(2,1,2);imshow(rErrorMap);
sDiffMap = rErrorMap;
