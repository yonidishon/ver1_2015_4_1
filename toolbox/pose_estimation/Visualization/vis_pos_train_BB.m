% TODO - THIS SCRIPT ISN'T WORKING!!!
% NOT COMPLETED
...
BB_folder ='\\cgm10\D\head_pose_estimation\2015_19_12_new_train_form';
...
pos_file = dir(fullfile(BB_folder,'train_pos*'));
neg_file = dir(fullfile(BB_folder,'train_neg*'));
if (isempty(pos_file)||isempty(neg_file))
    error('couldn''t find input files');
end
pos_file = pos_file.name;
neg_file = neg_file.name;
posid = fopen(fullfile(BB_folder,pos_file));
negid = fopen(fullfile(BB_folder,neg_file));
tline = fgetl(posid);
