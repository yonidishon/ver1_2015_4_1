...
for ii=1:length(avail_ch)
    ...
    % extract the ave_prob_map
    whole_prob = load(fullfile(src_fold,'saved_results',['prob_NN_whole_',avail_ch{ii}]));
    whole_prob = whole_prob.im_sum;
    % going to column representation
    whole_prob_col = im2col(whole_prob,[patchSz patchSz],'distinct');
    % Normalizing each patch
    %whole_prob_col = whole_prob_col./repmat(max(whole_prob_col),size(whole_prob_col,1),1);
    % changing the columns to be the same index as quadrants 
    whole_prob_col(:,[2,3,1,4]) = whole_prob_col;
    ...
    IDX(:,1) = quad_ind;
    % Find the nearest neigbour of the patch information in in the
    % ave patches prob
    idx = knnsearch(whole_prob_col',patch_data,'Distance','correlation');
    %             % debug
    %             figure();
