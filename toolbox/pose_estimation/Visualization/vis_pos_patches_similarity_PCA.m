...
for k=1:length(train_set);
    ...
    SKIP = round((size(movfiles,1)/2-OFFSET)/FRPERMOV);
    for ii=OFFSET:SKIP:length(gazeinfo.points);
        ...
        f_m=imread(fullfile(src_folder,train_set{k},sprintf('%06d_PCAm.png',ii)));
        f_s=imread(fullfile(src_folder,train_set{k},sprintf('%06d_PCAs.png',ii)));
        ...
        lu_quad_s = f_s(max(cent(1)-BB_size/2,1):cent(1)-1,max(cent(2)-BB_size/2,1):cent(2)-1);
        ru_quad_s = f_s(max(cent(1)-BB_size/2,1):cent(1)-1,cent(2):min(cent(2)+BB_size/2-1,size(f_m,2)));
        ld_quad_s = f_s(cent(1):min(cent(1)+BB_size/2-1,size(f_m,1)),max(cent(2)-BB_size/2,1):cent(2)-1);
        rd_quad_s = f_s(cent(1):min(cent(1)+BB_size/2-1,size(f_m,1)),cent(2):min(cent(2)+BB_size/2-1,size(f_m,2)));  
        %debug
%         figure();
