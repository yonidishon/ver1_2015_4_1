function []=func_Config_txt_CreateCRCNS(EXP_FOLD_NM,hosts)
pose_path = '\\cgm10\D\head_pose_estimation';
config_path = fullfile(pose_path,'config_files');
% Configuration Options:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
test_fnm_suf ='test';%'test_post';%'test';
resultFoldforPost ='CRCNSpng';%'origandPCAmPCAs_15_float'; %'origandPCAmPCAs_15';
resultFoldforHard = '';
FORESTSZ = 10;%8;%15;%10
...
SCALE_FC_IM = 2^8-1; % scale factor for final prediction map (TODO -needs to see if it is nessecary)
...
outputpath = fullfile('\\cgm10\D\head_pose_estimation\PredictionsCRCNS',sprintf('pred_%s',EXP_FOLD_NM));
...
for ii=1:length(hosts)
    ...
    fid=fopen(fullfile(config_path,sprintf('config_%s_%s_%s%s.txt',EXP_FOLD_NM,'CRCNS',hosts{ii},phase{jj})),'w');
    ...
    fprintf(fid,'# File with names of images\n%s\n',...
        fullfile('\\cgm10\D\head_pose_estimation\PredictionsCRCNS',...
        sprintf('%s_CRCNS%s.txt',hosts{ii},test_fnm_suf)));
