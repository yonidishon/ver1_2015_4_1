...
resFolder='\\cgm10\D\DIEM\scores\';
predFolder ='\\cgm10\D\head_pose_estimation\Predictions';
...
for im = 1:length(measures)
    ...
    for i = 1:nt
            ...
            tmp = matfile(fullfile(predFolder,methods{2},suffixfold,[videos{testIdx(i)},'_similarity.mat']));
            sim{i}=zeros(length(methods),nmeas,size(tmp.sim,3));
            sim_length = size(tmp.sim,3);
            sim_height = size(tmp.sim,2);
            clear tmp
        for k=1:length(methods)
             if exist(fullfile(predFolder,methods{k},suffixfold,[videos{testIdx(i)},'_similarity.mat']),'file')
