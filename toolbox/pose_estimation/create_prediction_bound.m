...
finalResultRoot = '\\cgm10\D\head_pose_estimation\Predictions\BOUND_RESULT_EVAL';


%% Training and testing settings
videos = movie_list([my_training_set,borji_list_subset]);
...
for ii=2:length(videos) % run for the length of the defined exp.
    try % MAIN ROUTINE to do.
        ...
        if ~exist(fullfile(finalResultRoot,vidnameonly),'dir')
            if ~(mkdir(fullfile(finalResultRoot,vidnameonly)))
                error('create_prediction_bound: couldn''t create a folder %s',vidnameonly);
            end
        end
        try
            
            indFr = 1:min(videoLen,length(gazeData.points));
            offset = 0;
            for ifr=1:length(indFr)
             fr = read(vr,indFr(ifr)+offset);
             gazeData.index = indFr(ifr)+offset;
...
%              end
             ...
             imwrite(predMap,fullfile(finalResultRoot,vidnameonly,sprintf('%06i_predMap.png',indFr(ifr)+offset)),'png');
            end
        catch me
            if visVideo
                close(vw);
