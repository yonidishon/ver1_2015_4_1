...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) 
{
	...
	int patch_sz = mxGetScalar(prhs[1]);
	...
	if (nrows<patch_sz || ncols<patch_sz)
	...
	double tmp;
	// create output var
	rows_out = patch_sz*patch_sz*ndims;
	cols_out = (nrows-patch_sz+1)*(ncols-patch_sz+1);
	plhs[0] = mxCreateDoubleMatrix(rows_out, cols_out, mxREAL);
	double *OUT = mxGetPr(plhs[0]);		
	// Loop over all blocks
	for (int c = 0 ; c < ncols-patch_sz+1 ; c++) // cols	
	{
		double *im_c = &(im[c*nrows]);
		for (int r = 0 ; r < nrows-patch_sz+1 ; r++) //rows
		{
			...
			for (int d = 0 ; d < ndims ; d++) //dims
			{
				...
				for(int cc = 0 ; cc < patch_sz ; cc++)
				{
					double *im_c_r_d_cc = &(im_c_r_d[cc*nrows]);
					for(int rr = 0 ; rr < patch_sz ; rr++)					
					{							
						*(OUT++) = im_c_r_d_cc[rr];							
					}
				}
