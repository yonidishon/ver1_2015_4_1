function model = adaBoostTrain( X0, X1, Weights, varargin )
...
if ~isempty(Weights)
    data.wts0=Weights(1:N0);
    data.wts1=Weights(N0+1 : N0+N1); 
    assert(length(Weights)== N0+N1);
end
...
for i=1:nWeak
  ...
  H1=H1+h1*alpha;
  if ~isempty(Weights)
      data.wts0=exp( H0).*Weights(1:N0);
      data.wts1=exp(-H1).*Weights(N0+1:end);
  else
